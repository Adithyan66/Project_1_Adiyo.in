...
import React, { useState, useEffect } from 'react';
import { X, MapPin, Home, Briefcase, ShoppingBag, Navigation, ArrowLeft } from 'lucide-react';
import axios from 'axios';
import { toast } from 'react-toastify';
import { createAddress, updateAddress } from '../../../services/profileService';
...
const AddEditAddressModal = ({ setShowModal, addressToEdit = null }) => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center z-50">
            {/* Semi-transparent backdrop */}
            <div className="absolute inset-0 bg-black opacity-50"></div>
            <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50 p-2 md:p-4">
                <div className="bg-white rounded-lg shadow-2xl w-full h-full md:h-auto md:max-w-5xl md:max-h-[90vh] border border-gray-200 flex flex-col">
                    <div className="flex items-center justify-between p-3 md:p-4 border-b border-gray-100 sticky top-0 bg-white z-10">
                        {isMobileView && activeTab === 'address' ? (
                            <button
                                onClick={() => setActiveTab('contact')}
                    ...
                    <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
                        ...
                        {isMobileView && (
                            <div className="px-4 py-2 flex justify-between items-center border-b border-gray-200">
                                <div className="flex items-center">
                                    <span className={`w-6 h-6 rounded-full inline-flex items-center justify-center mr-2 ${activeTab === 'contact' ? 'bg-black text-white' : 'bg-gray-200'}`}>1</span>
                                    <span className={`text-sm ${activeTab === 'contact' ? 'font-medium' : 'text-gray-500'}`}>Contact</span>
                                </div>
                                <div className="flex-1 mx-2 h-1 bg-gray-200">
                                    <div className={`h-full bg-black ${activeTab === 'address' ? 'w-full' : 'w-0'} transition-all duration-300`}></div>
                                </div>
                                <div className="flex items-center">
                                    <span className={`w-6 h-6 rounded-full inline-flex items-center justify-center mr-2 ${activeTab === 'address' ? 'bg-black text-white' : 'bg-gray-200'}`}>2</span>
                                    <span className={`text-sm ${activeTab === 'address' ? 'font-medium' : 'text-gray-500'}`}>Address</span>
                                </div>
                            </div>
                        )}
                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                {activeTab === 'contact' && (
                                    <div>
                                        ...
                                        {/* Mobile use current location button */}
                                        {isMobileView && (
                                            <div className="mt-6 p-3 bg-gray-50 rounded-md">
                                                <button
                                                    type="button"
                                        ...
                                        <div className="mt-8 flex justify-between">
                                            {!isMobileView && (
                                                <button
                                                    ...
                                                    onClick={getCurrentLocation}
                                                    disabled={isLoadingLocation}
                                                    className="px-4 py-2 flex items-center justify-center border border-black text-black rounded-md hover:bg-gray-50"
                                                >
                                                    <Navigation className="mr-2" size={16} />
                                                    {isLoadingLocation ? 'Detecting...' : 'Use current location'}
                                                </button>
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => setActiveTab('address')}
                                                className={`px-6 py-2 bg-black text-white rounded-md hover:bg-gray-800 ${isMobileView ? 'w-full' : ''}`}
                                            >
                                                Continue
                                            </button>
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    <button
                                                        type="button"
                                                        className={`flex items-center px-3 py-2 rounded-md ${formData.addressType === 'Home'
                                                            ? 'bg-black text-white'
                                                            : 'border border-gray-300 text-gray-700'
                                                            }`}
                                                        onClick={() => handleAddressTypeSelect('Home')}
                                                    ...
                                                    <button
                                                        type="button"
                                                        className={`flex items-center px-3 py-2 rounded-md ${formData.addressType === 'Work'
                                                            ? 'bg-black text-white'
                                                            : 'border border-gray-300 text-gray-700'
                                                            }`}
                                                        onClick={() => handleAddressTypeSelect('Work')}
                                                    ...
                                                    <button
                                                        type="button"
                                                        className={`flex items-center px-3 py-2 rounded-md ${formData.addressType === 'Other'
                                                            ? 'bg-black text-white'
                                                            : 'border border-gray-300 text-gray-700'
                                                            }`}
                                                        onClick={() => handleAddressTypeSelect('Other')}
                                        ...
                                        <div className="mt-6 flex flex-col md:flex-row justify-between gap-3">
                                            ...
                                            <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                                                ...
                                                <button
                                                    type="submit"
                                                    className="px-6 py-2 bg-black text-white rounded-md hover:bg-gray-800 w-full md:w-auto"
                                                >
                                                    {isEditMode ? 'Save Changes' : 'Add Address'}
                                                </button>
                                            </div>
