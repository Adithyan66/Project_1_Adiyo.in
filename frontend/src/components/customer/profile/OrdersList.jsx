...
const OrdersList = () => {
    ...
    return (
        <div className="flex-1 p-3 sm:p-6 bg-white mx-2 sm:m-6 rounded-md shadow-sm min-h-[800px]">
            {/* Header */}
            <div className="bg-gray-50 p-4 sm:p-6 border-b border-gray-200">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between space-y-2 sm:space-y-0">
                    <div className="flex items-center">
                        <ShoppingBag className="mr-3 text-gray-800" size={24} />
                        <h2 className="text-xl sm:text-2xl font-semibold text-gray-900">My Orders</h2>
                    </div>
                    <div className="flex items-center">
            ...
            <div className="p-4 sm:p-6 border-b border-gray-200 bg-gray-50">
                <div className="flex flex-col space-y-3 sm:space-y-0 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="w-full sm:w-auto">
                        <select
                            className="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:border-transparent appearance-none bg-white w-full"
                            value={statusFilter}
                            onChange={(e) => setStatusFilter(e.target.value)}
                        >
                            <option value="all">All Orders</option>
            ...
            {currentOrders.length > 0 ? (
                    ...
                    {currentOrders.map(order => (
                        ...
                        >
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between">
                                ...
                                <div className="flex flex-row items-center justify-between sm:justify-start sm:space-x-6">
                                    <StatusBadge status={order.orderStatus} />
                                    <div className="text-gray-900 font-semibold">₹{order.totalAmount.toFixed(2)}</div>
                            ...
                            <div className="mt-4 space-y-3 sm:space-y-0 sm:grid sm:grid-cols-3 sm:gap-4">
                                {order.orderItems.slice(0, 3).map(item => (
                                    <div key={item._id} className="flex items-center">
                                        <div className="h-20 w-20 sm:h-16 sm:w-16 bg-gray-100 rounded-md overflow-hidden mr-3 border border-gray-200">
                                            {item.product && item.product.colors && item.product.colors.length > 0 ? (
                                                <img
                                                    src={
                                                        item.product.colors.find(c => c.color.toLowerCase() === item.color.toLowerCase())?.images[0] ||
                ...
                <div className="text-center py-10 sm:py-16">
                    ...
                    <p className="text-lg sm:text-xl text-gray-700 font-medium mb-2">No orders found</p>
            ...
            {filteredOrders.length > 0 && (
                <div className="flex flex-col items-center mt-6">
                    <hr className="w-full border-t border-gray-300 my-8 sm:my-14" />
                    <div className="flex items-center space-x-2 sm:space-x-4">
                        <button
                            onClick={goToPreviousPage}
                            disabled={currentPage === 1}
                            className="px-3 sm:px-4 py-2 bg-gray-200 text-gray-700 rounded-lg disabled:opacity-50 hover:bg-gray-300 transition-colors duration-200 flex items-center"
                        >
