        ...
        return (
            ...
            >
                <img
                    ...
                    className="w-full h-full object-contain transition-transform duration-300"
                    style={{
                        transform: !isMobile && isHovering ? 'scale(1.8)' : 'scale(1)',
                        transformOrigin: 'center center'
                    }}
                />
            </div>
        ...
        <div className={`max-w-6xl mx-auto px-4 py-8 ${isMobile ? 'mt-[80px]' : 'mt-[100px]'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="flex flex-col items-center">
                    ...
                    {/* Thumbnails */}
                    <div className={`flex mt-4 self-start ${isMobile ? 'overflow-x-auto w-full pb-2 no-scrollbar' : 'space-x-4'}`}>
                        {selectedColor &&
                            selectedColor.images.map((image, index) => (
                                <div
                                    key={index}
                                    className={`border ${selectedImage === image ? "border-black" : "border-gray-300"} rounded p-1 cursor-pointer ${isMobile ? 'flex-shrink-0 mr-2' : ''}`}
                                    onClick={() => setSelectedImage(image)}
                                >
                                    <img
                                        src={image}
                                        alt={`Thumb ${index + 1}`}
                                        className={`${isMobile ? 'w-16' : 'w-20'} h-auto object-cover`}
                                    />
                                </div>
                            ))}
                    </div>
                    ...
                    {!isMobile && (
                        <div className="mt-12 flex space-x-4">
                            ...
                            <button
                                className="bg-black text-white px-8 py-3 ml-5 text-lg rounded-md hover:bg-gray-800 transition-colors hover:cursor-pointer"
                ...
                <div>
                    ...
                    <div className="mt-6">
                        <h3 className="font-semibold text-gray-500">Choose Size</h3>
                        <div className={`flex mt-2 ${isMobile ? 'overflow-x-auto pb-2 no-scrollbar' : 'flex-wrap gap-3'}`}>
                            {availableSizes.length > 0 ? (
                                availableSizes.map((size, index) => (
                                    <button
                                        key={index}
                                        onClick={() => setSelectedSize(size)}
                                        className={`px-4 py-2 border border-gray-300 rounded-3xl hover:cursor-pointer ${selectedSize === size ? 'bg-black text-white' : 'bg-gray-100'} ${isMobile ? 'flex-shrink-0 mr-3' : ''}`}
                                    >
                                        {size}
                                    </button>
                                ))
                                ...
                                {availableOffers.map((offer, index) => (
                                    <li key={index} className="rounded-md">
                                        <div className="flex flex-wrap">
                                            <img src={greenArrow} alt="" className="w-4 h-4 mr-3 mt-1" />
                                            <div>
                                                <span className="font-semibold mr-1">{offer.name}:</span>{" "}
                                                {offer.discount}% off
                                                {" "}
                                                <span className="text-sm text-gray-600 block md:inline mt-1 md:mt-0 md:ml-2">
                                                    (Valid until {new Date(offer.endDate).toLocaleDateString()})
                                                </span>
                                            </div>
                                        </div>
                    ...
                    <div className="mt-6 mb-20 md:mb-6">
                        ...
                        <div className="flex items-center space-x-2 mt-2">
                            <input
                                type="text"
                                placeholder="Enter pincode here"
                                className="border border-gray-300 rounded px-3 py-2 focus:outline-none w-full md:w-auto"
                            />
                            <button className="px-4 py-2 border border-gray-300 rounded-md bg-gray-200 hover:bg-gray-300 hover:cursor-pointer whitespace-nowrap">
                                Check
                            </button>
                        </div>
                        <p className="text-gray-600 mt-2">Delivery by 22 Feb, Saturday</p>
            ...
            {isMobile && (
                <div className="fixed bottom-0 left-0 right-0 bg-white shadow-lg border-t border-gray-200 flex z-50">
                    <button
                        className="flex-1 py-4 text-center bg-white border-r border-gray-300 font-semibold"
                        onClick={handleAddtoCart}
                        disabled={isLoading}
                    >
                        {isLoading ? "Processing..." : isInCart ? "View Cart" : "Add to Cart"}
                    ...
                    <button
                        className="flex-1 py-4 text-center bg-black text-white font-semibold"
                        onClick={handleBuynow}
                    >
                        Buy Now
                    </button>
