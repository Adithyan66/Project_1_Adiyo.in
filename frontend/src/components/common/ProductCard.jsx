...
function ProductCard({ product }) {
  ...
  return (
    <div
      className="rounded-lg p-2 sm:p-4 flex flex-col bg-white shadow-sm hover:shadow-lg transition-all duration-300 relative cursor-pointer"
      onClick={() => {
        navigate(`/product-detail/${product._id}`);
      }}
    >
      <div className="relative">
        <img
          src={product?.colors[0].images[0]}
          alt={product?.name}
          className="w-full h-54 sm:h-80 md:h-94 object-cover mb-2 sm:mb-3 mx-auto rounded-lg"
        />
      </div>
      ...
      <div className="flex flex-col flex-grow">
        ...
        <h6 className="text-xs sm:text-sm font-semibold text-gray-400 line-clamp-1">
          {getCategoryName(product.category, product.subCategory)}
        </h6>

        {/* Rating */}
        <div className="flex items-center mt-1">
          <div className="text-yellow-400 mr-2 text-sm sm:text-base">
            {Array(5).fill().map((_, i) => (
              <span key={i}>
                {i < Math.floor(rating) ? "★" : i < rating ? "★" : "☆"}
              </span>
        ...
        <div className="mt-2 sm:mt-3 flex items-center">
          <div className="flex-grow">
            <span className="font-bold text-xl sm:text-2xl text-gray-900">₹{product?.minDiscountPrice}</span>
            {product?.colors[0].basePrice && (
              <span className="text-xs sm:text-sm text-gray-500 ml-1 sm:ml-2 line-through">
                ₹{product?.colors[0].basePrice}
              </span>
            )}
          </div>
          ...
          {product?.colors[0].discountPercentage > 0 && (
            <span className="text-green-800 text-sm sm:text-xl py-1 px-2 rounded-full font-medium">
              {Math.ceil(product?.colors[0].discountPercentage)}% off
            </span>
          )}
        </div>
...
function ProductCardShimmer() {
  return (
    <div className="rounded-lg p-3 sm:p-4 flex flex-col bg-white shadow-sm relative overflow-hidden">
      {/* Shimmer overlay */}
      <div className="absolute inset-0 -translate-x-full animate-[shimmer_1.5s_infinite] bg-gradient-to-r from-transparent via-white/20 to-transparent" />

      {/* Image placeholder */}
      <div className="relative">
        <div className="w-full h-64 sm:h-80 md:h-95 bg-gray-200 rounded-lg mb-2 sm:mb-3 mx-auto" />
      </div>
      ...
      <div className="flex flex-col flex-grow">
        ...
        {/* Rating placeholder */}
        <div className="flex items-center mt-1">
          <div className="h-3 sm:h-4 bg-gray-200 rounded w-20 sm:w-24 mr-2"></div>
