...
//     // Local state for customer's active status
//     const [isActive, setIsActive] = useState(customer.isActive);

//     // If the customer prop changes, update the local state
//     useEffect(() => {
...
//         try {
//             const response = await axios.patch(`${API_BASE_URL}/admin/block-user/${customer._id}?isActive=${!customer.isActive}`)
//         }
//         catch (error) {
...
export default function CustomerRightSection({ customer }) {
    ...
    // Example stats and transactions...
    const totalOrders = 2400;
    const totalOrdersGrowth = 23; // +23% this month
    const totalBalance = 10000;
    const totalBalanceGrowth = 10; // +10% vs last month
    ...
    // Updated block/unblock handler that updates local state
    const handleBlock = async () => {
        try {
            // Toggle active state by sending the inverse of current state
            await axios.patch(
                `${API_BASE_URL}/admin/block-user/${customer._id}?isActive=${!isActive}`
            );
            // Update local state to re-render component with new status
                ...
                <div className="w-full md:w-1/3 bg-white rounded shadow">
                    ...
                    <div className="px-4 pt-12 pb-4 text-center">
                        ...
                        <div className="mt-6 space-y-4 text-sm text-left">
                            {/* Customer ID */}
                            <div className="flex">
                                <img src={customerID} alt="Customer ID icon" className="h-8 w-8 mt-1" />
                                <div className="ml-4">
                                    <p className="font-semibold">Customer ID:</p>
                                    <p className="text-gray-700 ml-2">{customer.userId}</p>
                                </div>
                            ...
                            {/* Email */}
                            <div className="flex">
                                <img src={email} alt="Email icon" className="h-8 w-8 mt-1" />
                                <div className="ml-4">
                                    <p className="font-semibold">Email:</p>
                                    <p className="text-gray-700 ml-2">{customer.email}</p>
                                </div>
                            ...
                            {/* Address */}
                            <div className="flex">
                                <img src={address} alt="Address icon" className="h-8 w-8 mt-1" />
                                <div className="ml-4">
                                    <p className="font-semibold">Address:</p>
                                    <p className="text-gray-700 ml-2">{customer.address}</p>
                                </div>
                            ...
                            {/* Phone Number */}
                            <div className="flex">
                                <img src={phone} alt="Phone icon" className="h-8 w-8 mt-1" />
                                <div className="ml-4">
                                    <p className="font-semibold">Phone Number:</p>
                                    <p className="text-gray-700 ml-2">{customer.phone}</p>
                                </div>
                            ...
                            {/* Registration Date */}
                            <div className="flex">
                                <img src={LastTransaction} alt="Transaction icon" className="h-8 w-8 mt-1" />
                                <div className="ml-4">
                                    <p className="font-semibold">Registration Date:</p>
                                    <p className="text-gray-700 ml-2">{formattedDate}</p>
                                </div>
                            ...
                            {/* Last Online */}
                            <div className="flex">
                                <img src={lastonline} alt="Transaction icon" className="h-8 w-8 mt-1" />
                                <div className="ml-4">
                                    <p className="font-semibold">Last online:</p>
                                    <p className="text-gray-700 ml-2">{customer.lastonline}</p>
                                </div>
                ...
                <div className="w-full md:w-2/3 flex flex-col gap-6">
                    ...
                    <div className="bg-white rounded shadow">
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="text-gray-700">
                                {currentTx.map((tx) => (
                                    <tr key={tx.orderId} className="border-b hover:bg-gray-50 transition-colors">
                                        <td className="py-3 px-4">{tx.orderId}</td>
                                        <td className="py-3 px-4">{tx.product}</td>
                                        <td className="py-3 px-4">
                                            ₹
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                {Array.from({ length: totalTxPages }, (_, i) => i + 1).map((pageNum) => (
                                    <button
                                        key={pageNum}
                                        onClick={() => setCurrentTxPage(pageNum)}
                                        className={`px-3 py-1 rounded ${currentTxPage === pageNum ? "bg-black text-white" : "bg-gray-100"}`}
                                    >
                                        {pageNum}
                                    </button>
                                ))}
