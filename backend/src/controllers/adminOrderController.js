...
export const getOrders = async (req, res) => {
    try {
        ...
        res.status(OK).json({
            success: true,
            message: messages.ORDER.FETCHED_SUCCESSFULLY,
            orders,
            totalOrders
        });
    } catch (err) {
        ...
        res.status(INTERNAL_SERVER_ERROR).json({
            success: false,
            message: messages.ORDER.FAILED_FETCH
        });
    }
};
...
export const getOrderDetails = async (req, res) => {
    try {
        ...
        res.status(OK).json({
            success: true,
            message: messages.ORDER.DETAILS_FETCHED_SUCCESSFULLY,
            order: formattedOrder
        });
    } catch (error) {
        console.error('Error fetching order details:', error);
        res.status(INTERNAL_SERVER_ERROR).json({
            success: false,
            message: messages.ORDER.FAILED_FETCH_DETAILS
        });
    }
};
...
export const updateOrderStatus = async (req, res) => {
    try {
        ...
        if (!validStatuses.includes(status)) {
            return res.status(BAD_REQUEST).json({
                success: false,
                message: messages.ORDER.INVALID_STATUS
            });
        }
        ...
        res.status(OK).json({
            success: true,
            message: messages.ORDER.STATUS_UPDATED_SUCCESSFULLY,
            order: formattedOrder
        });
    } catch (error) {
        console.error('Error updating order status:', error);
        res.status(INTERNAL_SERVER_ERROR).json({
            success: false,
            message: messages.ORDER.FAILED_UPDATE_STATUS
        });
    }
};
...
export const handleReturnVerification = async (req, res) => {
    try {
        ...
        if (order.returnStatus !== 'requested') {
            return res.status(BAD_REQUEST).json({
                success: false,
                message: messages.ORDER.RETURN_NO_REQUEST
            });
        }
        ...
        res.status(OK).json({
            success: true,
            message: approved ? messages.ORDER.RETURN_APPROVED_SUCCESSFULLY : messages.ORDER.RETURN_REJECTED,
            order: formattedOrder,
        });
    } catch (error) {
        console.error('Error processing return verification:', error);
        res.status(INTERNAL_SERVER_ERROR).json({
            success: false,
            message: messages.ORDER.FAILED_PROCESS_RETURN
        });
    }
};
...
export const verifyReturn = async (req, res) => {
    ...
    if (!productId || !userId || typeof approved !== "boolean") {
        return res.status(BAD_REQUEST).json({
            success: false,
            message: messages.ORDER.MISSING_REQUIRED_FIELDS
        });
    }
    ...
    try {
        ...
        if (order.orderStatus !== "return requested") {
            ...
            return res.status(BAD_REQUEST).json({
                success: false,
                message: messages.ORDER.NOT_PENDING_RETURN
            });
        }
        ...
        if (order.user?.toString() !== userId) {
            ...
            return res.status(BAD_REQUEST).json({
                success: false,
                message: messages.ORDER.USER_MISMATCH
            });
        }
        ...
        return res.status(OK).json({
            success: true,
            message: approved
                ? `${messages.ORDER.RETURN_VERIFIED_SUCCESSFULLY}. ₹${refundAmount.toFixed(2)} has been credited to the user's wallet.`
                : messages.ORDER.RETURN_VERIFIED_SUCCESSFULLY,
            refundAmount: approved ? refundAmount : 0
        });
    } catch (error) {
        await session.abortTransaction();
        ...
        return res.status(INTERNAL_SERVER_ERROR).json({
            success: false,
            message: messages.ORDER.FAILED_VERIFY_RETURN,
            error: error.message
        });
    }
};