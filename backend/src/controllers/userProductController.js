...
export const getNewArrivals = async (req, res) => {
    try {
        ...
        res.status(OK).json({
            success: true,
            message: messages.PRODUCT.NEW_ARRIVALS_FETCHED_SUCCESSFULLY,
            products: transformedProducts,
            total: await Product.countDocuments({ deletedAt: null })
        });
    } catch (error) {
        ...
        res.status(INTERNAL_SERVER_ERROR).json({
            success: false,
            message: messages.PRODUCT.FAILED_FETCH_NEW_ARRIVALS
        });
    }
};
...
export const getTopSellingProducts = async (req, res) => {
    try {
        ...
        res.status(OK).json({
            success: true,
            message: messages.PRODUCT.TOP_SELLING_FETCHED_SUCCESSFULLY,
            products: transformedProducts,
            total: await Product.countDocuments({ deletedAt: null })
        });
    } catch (error) {
        ...
        res.status(INTERNAL_SERVER_ERROR).json({
            success: false,
            message: messages.PRODUCT.FAILED_FETCH_TOP_SELLING
        });
    }
};
    ...
    try {
        ...
        if (req.query.category) {
            try {
                ...
                return res.status(BAD_REQUEST).json({
                    success: false,
                    message: messages.PRODUCT.INVALID_CATEGORY_ID
                });
            }
        }
        ...
        if (req.query.subCategory) {
            try {
                ...
                return res.status(BAD_REQUEST).json({
                    success: false,
                    message: messages.PRODUCT.INVALID_SUBCATEGORY_ID
                });
            }
        }
