...
export const salesReport = async (req, res) => {
    try {
        ...
        res.status(OK).json({
            success: true,
            message: messages.SALES_REPORT.GENERATED_SUCCESSFULLY,
            summary,
            data,
            pagination: {
                totalItems: totalCount,
        ...
        res.status(INTERNAL_SERVER_ERROR).json({
            success: false,
            message: messages.SALES_REPORT.FAILED_GENERATE
        });
    }
};
...
export const getDashboardData = async (req, res) => {
    try {
        ...
        if (timeFilter === 'yearly') {
            if (!year) {
                return res.status(BAD_REQUEST).json({
                    success: false,
                    message: messages.DASHBOARD.YEAR_REQUIRED
                });
            }
            ordersPipeline = [
                {
            ...
            if (!year || !month) {
                return res.status(BAD_REQUEST).json({
                    success: false,
                    message: messages.DASHBOARD.YEAR_MONTH_REQUIRED
                });
            }
            ordersPipeline = [
                {
            ...
            if (!startDate || !endDate) {
                return res.status(BAD_REQUEST).json({
                    success: false,
                    message: messages.DASHBOARD.DATES_REQUIRED
                });
            }
        ...
        return res.status(OK).json({
            success: true,
            message: messages.DASHBOARD.DATA_FETCHED_SUCCESSFULLY,
            data
        });
    } catch (error) {
        console.error('Error fetching dashboard data:', error);
        return res.status(INTERNAL_SERVER_ERROR).json({
            success: false,
            message: messages.DASHBOARD.FAILED_FETCH
        });
    }
};